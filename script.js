...
if (chartCanvas) {
    ...
    window.auctionCharts.push(liveChart);
}
...
// Failsafe Mobile Menu Functions
window.openMobileMenu = function() {
    const menu = document.getElementById('mobile-menu');
    const btn = document.getElementById('mobile-menu-btn');
    if (!menu) return;

    menu.classList.remove('translate-x-full', 'opacity-0', 'invisible', 'pointer-events-none');
...
window.closeMobileMenu = function() {
    ...
    menu.classList.add('translate-x-full', 'opacity-0', 'invisible', 'pointer-events-none');
    menu.classList.remove('pointer-events-auto');
    menu.setAttribute('data-state', 'closed');
    document.body.style.overflow = ''; // Use empty string to remove override
...
window.toggleMobileMenu = function() {
    ...
    // Use data-state and class list to verify true state of the menu
    const isOpen = menu.getAttribute('data-state') === 'open' || !menu.classList.contains('translate-x-full');
...
// Initial state cleanup and Sidebar initialization
document.addEventListener('DOMContentLoaded', () => {
    ...
    const btn = document.getElementById('mobile-menu-btn');
    if (btn) {
        btn.classList.remove('active'); // Ensure button resets on load
    }

    // Dashboard Sidebar Logic
    const dashboardSidebar = document.getElementById('dashboard-sidebar');
    
    window.toggleDashboardSidebar = function(forceClose = false) {
    ...
    if (dashboardSidebar) {
        ...
        sidebarLinks.forEach(link => {
            link.addEventListener('click', () => {
                if (window.innerWidth < 768) {
                    window.toggleDashboardSidebar(true);
                }
            });
        });
        ...
        document.addEventListener('click', (e) => {
            const sidebarOpenBtn = document.getElementById('open-sidebar-btn'); // Grab latest reference
            if (window.innerWidth < 768 && !dashboardSidebar.classList.contains('-translate-x-full')) {
                const clickedInsideSidebar = dashboardSidebar.contains(e.target);
                const clickedToggleButton = sidebarOpenBtn && sidebarOpenBtn.contains(e.target);
        ...
        window.addEventListener('resize', () => {
            if (window.innerWidth >= 768 && !dashboardSidebar.classList.contains('-translate-x-full')) {
                document.body.style.removeProperty('overflow');
                dashboardSidebar.classList.add('-translate-x-full', 'opacity-0', 'invisible');
            }
        });

        // Ensure icons render correctly with the new global DOM handler
        if (window.lucide) lucide.createIcons();
    }
});

// Link click handler (delegated for stability)
document.addEventListener('click', (e) => {
...
function toggleMobileDropdown(dropdownId, iconId) {
    ...
    if (dropdown.classList.contains('max-h-0')) {
        ...
        dropdown.classList.remove('max-h-60', 'opacity-100');
...
const initHealthCharts = () => {
    ...
    const data = [40, 45, 55, 65, 120, 50, 42, 38, 60, 40];
    const chart = new Chart(ctx, {
        ...
        options: {
            ...
            scales: {
                x: { grid: { color: colors.gridColor }, ticks: { color: colors.axisColor, font: { size: 11 } } },
                y: { grid: { color: colors.gridColor }, ticks: { color: colors.axisColor, font: { size: 11 } }, min: 20, max: 140 }
            }
        }
    });
    window.auctionCharts.push(chart);
